...
public class Utility {
	...
	public static int rcount(int bbsno, YrecoDAO rdao) {
		
		Map map = new HashMap();
		
		return rdao.total(map);
	}
	  /** 
     * 파일업로드 처리(model1,mvc) 
     * @param fileItem 
	...
	public static List<String> getDay(){
		...
		SimpleDateFormat sd = new SimpleDateFormat("yyy-MM-dd");//db의 날짜형태와 자바의 날짜형태 맞춰줘야함
		Calendar cal = Calendar.getInstance();//추상 ㅡㄹ래스여서 getinstance로 객체생성
		for(int j=0; j<3; j++){
...
//	페이징4
	...
	public static String paging2(int totalRecord, int nPage, int recordPerPage, String url, int ynum, int nowPage,
		...
		int _nowPage = (nowGrp - 1) * pagePerBlock; // 10개 이전 페이지로 이동
		if (nowGrp >= 2) {
		str.append("<span class='span_box_1'><A href='"+url+ "?ynum=" + ynum + "&nPage=" + _nowPage +"&col=" + col + "&word=" + word + "&nowPage="
		+ nowPage +"'>이전</A></span>");
		}
		...
		str.append("<span class='span_box_1'><A href='"+url+"?ynum=" + ynum + "&nPage=" + i + "&col=" + col + "&word=" + word + "&nowPage=" + nowPage
		+ "'>" + i + "</A></span>");
		}
		}
